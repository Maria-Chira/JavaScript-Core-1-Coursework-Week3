...
function findSafeOxygenLevel(array) {
  const safePlanet = array
    .filter((oxygen) => oxygen.includes("%"))
    .map((oxygen) => oxygen.replace("%", ""))
    .sort()
    .find((oxygen) => Number(oxygen) > 19.5 && Number(oxygen) < 23.5);
  return safePlanet ? safePlanet + "%" : undefined;
}
