...
function getTransportModes(array) {
  const transport = array.slice(1, array.length);
  return transport;
}
...
*/
function isAccessibleByTransportMode(transportArray, transport) {
  return transportArray.includes(transport);
}
...
/*
  Implement the function getLocationName that
   ...
   - Returns the name of the location
      e.g: "Tower Bridge"
*/
function getLocationName(arrayLocTras) {
  return arrayLocTras[0];
...
function journeyPlanner(locations, transportMode) {
  ...
  locations.map (location => {
    let locationName = getLocationName(location); // "Tower Bridge"
    let availableTransport = getTransportModes(location); // ["tube", "river boat"]
    let isAccessibleByTransport = isAccessibleByTransportMode(availableTransport,transportMode);// t/f
    if (isAccessibleByTransport){
      availableLoc.push(locationName);
    }
  } );
  return availableLoc;
